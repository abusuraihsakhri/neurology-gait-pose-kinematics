...
class AuditTrail:
    ...
    def __init__(self, secret_key: Optional[str] = None):
        ...
        if not resolved_key:
            import warnings
            warnings.warn(
                "AUDIT_SECRET_KEY not set. Using a development-only default. "
                "Set AUDIT_SECRET_KEY env var in production for secure audit signing.",
                stacklevel=2,
            )
            resolved_key = "DEV-ONLY-KEY-SET-AUDIT_SECRET_KEY-IN-PROD"
